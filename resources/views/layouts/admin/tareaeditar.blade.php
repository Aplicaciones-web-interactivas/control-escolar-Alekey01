...
<head>
    ...
    <title>Editar Tarea - Control Escolar</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="max-w-lg mx-auto px-6 py-10">
        ...
        <div class="bg-white rounded-2xl shadow p-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6">Editar tarea</h2>
            ...
            <form action="{{ route('tareas.update', $tarea->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="maestro_id" name="maestro_id" required
                        ...
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ old('maestro_id', $tarea->maestro_id) == $usuario->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="alumno_id" name="alumno_id" required
                        ...
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ old('alumno_id', $tarea->alumno_id) == $usuario->id ? 'selected' : '' }}>
                                {{ $usuario->nombre }} ({{ $usuario->clave_institucional }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="estado" name="estado" required
                        ...
                        <option value="entregada"  {{ old('estado', $tarea->estado) === 'entregada'  ? 'selected' : '' }}>Entregada</option>
                        <option value="calificada" {{ old('estado', $tarea->estado) === 'calificada' ? 'selected' : '' }}>Calificada</option>
                    </select>
                </div>
